...
const GitHubProjectData = ({ project }) => {
    ...
    useEffect(() => {
        const fetchData = (project) => {
            axios.get(`https://api.github.com/repos/${GITHUB_USERNAME}/${project.name}`)
                .then(result => {
    ...
    return (
        <>
            [<a href={`https://github.com/${GITHUB_USERNAME}/${project.name}`}><b>{`${GITHUB_USERNAME}/${project.name}`}</b></a>]: {description}
            <br />
            {projectWebsite && [<>Homepage: <a href={projectWebsite}>{projectWebsite}</a><br /></>]}
            {starsCount > 0 && (<div className={githubProjectDataStyles.metrics}>
                <span className={githubProjectDataStyles.metric}>Stars</span>:{" "}
                <a href={starsgazersURL} className={githubProjectDataStyles.metricCount}> {starsCount}</a> •{" "}
                <span className={githubProjectDataStyles.metric}>Forks</span>:{" "}
                <a href={forksURL} className={githubProjectDataStyles.metricCount}> {forksCount}</a>
            </div>)}
        </>
    )
}
